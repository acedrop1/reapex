...
// Full wordmark SVG logo — "reapex" text
function LogoSVG({ width = 150, height = 34, color = '#FFFFFF' }: { width?: number; height?: number; color?: string }) {
  return (
    <svg
...
// Icon mark SVG — the "X" symbol
function IconSVG({ size = 40, color = '#FFFFFF' }: { size?: number; color?: string }) {
  return (
    <svg
      xmlns="http://www.w3.org/2000/svg"
      viewBox="0 0 86.94 100.26"
      width={size}
      height={size}
      style={{ display: 'block' }}
    >
      <g fill={color}>
        <path d="M19.86,99.49c-6.01.52-11.24.5-18.47.18l32.31-50.38L0,.65C6.97.01,12.57.04,19.41.15l33.04,47.99-32.59,51.35Z"/>
        <polygon points="68.06 100.26 52.09 76.13 61.27 61.68 86.2 99.49 68.06 100.26"/>
        <polygon points="62.36 36.44 52.37 22.49 68.19 0 86.94 .47 62.36 36.44"/>
      </g>
    </svg>
  );
}

export function ReapexLogo({
  width = 150,
  height = 34,
  variant = 'light'
}: {
...
export function ReapexSidebarLogo() {
  return (
    <Box sx={{ display: 'flex', alignItems: 'center', px: 2, py: 1 }}>
      <LogoSVG width={120} height={28} color="#FFFFFF" />
    </Box>
  );
}

// Collapsed sidebar icon logo
export function ReapexIconLogo({ size = 32, invert = false }: { size?: number; invert?: boolean }) {
  return (
    <Box sx={{ display: 'flex', alignItems: 'center', justifyContent: 'center', width: '100%', py: 1.5 }}>
      <IconSVG size={size} color={invert ? '#000000' : '#FFFFFF'} />
    </Box>
  );
}
