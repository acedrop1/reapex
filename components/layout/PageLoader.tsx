...
/**
 * Full-screen page loader with Reapex logo wordmark and gold progress bar.
 * Renders on initial dashboard load, then fades out and unmounts.
 */
export default function PageLoader() {
  ...
  useEffect(() => {
    // Trigger fade-out after the progress bar finishes
    const fadeTimer = setTimeout(() => setLoaded(true), 1000);
  ...
  return (
    ...
    >
      {/* Reapex wordmark logo */}
      <Box
        sx={{
          mb: 3,
          opacity: 0,
          transform: 'translateY(16px)',
          animation: 'loaderLogoIn 0.6s ease-out 0.15s forwards',
      ...
      <Box
        sx={{
          ...
          overflow: 'hidden',
          opacity: 0,
          animation: 'loaderLogoIn 0.4s ease-out 0.4s forwards',
        }}
      >
        <Box
          sx={{
            height: '100%',
            width: 0,
            background: 'linear-gradient(90deg, #E2C05A, #c4a43e)',
            borderRadius: 2,
            animation: 'loaderBarFill 0.9s ease-out 0.5s forwards',
